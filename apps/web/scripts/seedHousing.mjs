...
const utaHousing = [
  ...
  },
  {
    ...
    priceLevel: "$$",
    officialFeatures: ["Pool", "Gym", "Study Area", "Student Living", "Near Campus"],
...
const rows = utaHousing.map((item) => ({
  slug: item.id,
  name: item.name,
  category: item.category,
  source: item.source,
  address: item.address,
  short_location: item.shortLocation,
  description: item.description,
  image: item.image,
  price_level: item.priceLevel,
  official_features: item.officialFeatures,
  tags: item.tags,
  official_url: item.officialUrl,
  rating: item.rating,
  review_count: item.reviewCount,
  distance_from_uta: item.distanceFromUTA,
  type: item.category === "apartment" ? "Apartment" : "Residence Hall",
...
async function seedHousing() {
  ...
  console.log("Seeded buildings:", data?.length ?? 0);
}
